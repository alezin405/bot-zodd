...
async function displayHeader() {
  const header = [
    `${colors.bold}🚀  - Atualizador${colors.reset}`,
    `${colors.bold}👨‍💻 Criado por ${colors.reset}`,
  ];
...
async function checkRequirements() {
  ...
  try {
    ...
    printWarning('⚠️ Git não encontrado! É necessário para atualizar o .');
    if (isWindows) {
...
async function downloadUpdate() {
  printMessage('📥 Baixando a versão mais recente do ...');
...
async function main() {
  ...
  try {
    ...
    const response = await fetch('https://api.github.com/repos/y//commits?per_page=1', {
