...
const codeMode = process.argv.includes('--code') || process.env._CODE_MODE === '1';
...
async function createGroupMessage(Sock, groupMetadata, participants, settings, isWelcome = true) {
    ...
    if (settings.image) {
        ...
        if (participants.length === 1 && isWelcome) {
            profilePicUrl = await Sock.profilePictureUrl(participants[0], 'image').catch(() => profilePicUrl);
        }
...
async function handleGroupParticipantsUpdate(Sock, inf) {
    try {
        ...
        let groupMetadata = await Sock.groupMetadata(from).catch(err => {
        ...
        switch (inf.action) {
            case 'add': {
                ...
                if (membersToRemove.length > 0) {
                    await Sock.groupParticipantsUpdate(from, membersToRemove, 'remove').catch(err => {
                ...
                if (membersToWelcome.length > 0) {
                    const message = await createGroupMessage(Sock, groupMetadata, membersToWelcome, groupSettings.welcome || {
                        text: groupSettings.textbv
                    });
                    
                    await Sock.sendMessage(from, message).catch(err => {
            ...
            case 'remove': {
                if (groupSettings.exit?.enabled) {
                    const message = await createGroupMessage(Sock, groupMetadata, inf.participants, groupSettings.exit, false);
                    await Sock.sendMessage(from, message).catch(err => {
...
async function fetchLidWithRetry(Sock, jid, maxRetries = 3) {
...
async function fetchLidsInBatches(Sock, uniqueJids, batchSize = 5) {
    const lidResults = [];
    ...
    for (let i = 0; i < uniqueJids.length; i += batchSize) {
        ...
        const batchPromises = batch.map(jid => fetchLidWithRetry(Sock, jid));
...
async function updateOwnerLid(Sock) {
    const ownerJid = `${numerodono}@s.whatsapp.net`;
    try {
        const result = await fetchLidWithRetry(Sock, ownerJid);
        if (result) {
...
async function performMigration(Sock) {
    let scanResult;
    try {
    ...
    const { jidToLidMap, successfulFetches } = await fetchLidsInBatches(Sock, uniqueJids);
...
async function createBotSocket(authDir) {
    try {
        ...
        if (codeMode && !Sock.authState.creds.registered) {
            ...
            const code = await Sock.requestPairingCode(phoneNumber.replaceAll('+', '').replaceAll(' ', '').replaceAll('-', ''));
        ...
        Sock.ev.on('group-participants.update', async (inf) => {
            await handleGroupParticipantsUpdate(Sock, inf);
        });
        ...
        const processMessage = async (info) => {
            ...
            if (typeof indexModule === 'function') {
                await indexModule(Sock, info, null, messagesCache, rentalExpirationManager);
            } else {
        ...
        Sock.ev.on('connection.update', async (update) => {
            ...
            if (qr && !Sock.authState.creds.registered && !codeMode) {
            ...
            if (connection === 'open') {
                ...
                await updateOwnerLid(Sock);
                await performMigration(Sock);
                
                rentalExpirationManager.nazu = Sock;
